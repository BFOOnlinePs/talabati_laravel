...
class StoreLogic
{
    public static function get_stores($zone_id, $filter_data, $type, $store_type, $limit = 10, $offset = 1, $featured = false, $longitude = 0, $latitude = 0, $filter = null, $rating_count = null)
    {
        ...
        $query = Store::type($type)->withOpen($longitude ?? 0, $latitude ?? 0)
    ...
    public static function get_latest_stores($zone_id, $limit = 50, $offset = 1, $type = 'all', $longitude = 0, $latitude = 0)
    {
        ...
        Log::info('aseel config(module.current_module_data)' . config('module.current_module_data'));
        ...
        $query = Store::withOpen($longitude ?? 0, $latitude ?? 0)
            ...
            ->when(config('module.current_module_data'), function ($query) use ($zone_id) {
                ...
                if ($currentModule) {
                    $query->whereHas('zone.modules', function ($query)  use ($currentModule) {
                        $query->where('modules.id', $currentModule['id']);
                    })->module($currentModule['id']);


                    Log::info('aseel config(module.current_module_data)[all_zone_service]' . $currentModule['all_zone_service']);

                    if (!$currentModule['all_zone_service']) {
                        $query->whereIn('zone_id', json_decode($zone_id, true));
                    }
                } else {
                    // If the module is null, still check the zone
                    $query->whereIn('zone_id', json_decode($zone_id, true));
                }
            })
            ->Active()
